...
Sudoku board may also contain 0's, which will represent empty cells. Boards containing one or more zeroes are considered to be invalid solutions.
...
export function validateSudoku(board: Board): boolean {
  ...
  for (let rowBlock = 0; rowBlock < 3; rowBlock++) {
    for (let colBlock = 0; colBlock < 3; colBlock++) {
      ...
      for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
          block.push(board[rowBlock * 3 + i][colBlock * 3 + j]);
        }
      }
