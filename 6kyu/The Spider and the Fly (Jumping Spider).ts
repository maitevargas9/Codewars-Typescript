...
export function spiderToFly(spider: string, fly: string): number {
  const toCartesian = (coord: string): [number, number] => {
    const radial = coord[0];
    const ring = parseInt(coord[1]);

    if (ring === 0) {
      return [0, 0];
    }

    const letterIndex = radial.charCodeAt(0) - "A".charCodeAt(0);
    const angleRad = Math.PI / 2 - (letterIndex * Math.PI) / 4;

    return [ring * Math.cos(angleRad), ring * Math.sin(angleRad)];
  };

  const [sx, sy] = toCartesian(spider);
  const [fx, fy] = toCartesian(fly);
