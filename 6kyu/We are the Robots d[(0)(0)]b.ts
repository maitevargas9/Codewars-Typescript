...
It's pretty random actually but with a global supply chain it's hard to control which part you get. Most of the parts are made global 
except the ones made in the factory which do not change.
...
There may be cases where a part is totally missing and of course a robot cannot function at all without a part or where the factory put a 
valid part in the wrong place and it's again not a valid robot.
...
export function countRobots(a: string[]): string[] {
  ...
  const robotPattern = new RegExp(
    `${L}${B}${B}${E}${B}${B}${E}${B}${B}${L}`,
    "g"
  );

  let automatikCount = 0;
  let mechanikCount = 0;

  for (const str of a || []) {
    const lower = str.toLowerCase();
    const robots = (str.match(robotPattern) || []).length;

    if (lower.includes("automatik")) {
      automatikCount += robots;
    } else if (lower.includes("mechanik")) {
      mechanikCount += robots;
    }
  }

  return [
    `${automatikCount} robots functioning automatik`,
    `${mechanikCount} robots dancing mechanik`
  ];
}
